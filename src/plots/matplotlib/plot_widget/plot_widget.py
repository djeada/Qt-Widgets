...
class PlotWidget(FigureCanvas, PlotWidgetInterface):
    ...
    def style(self):
        ...
        font = Font(family=matplotlib.rcParams["font.family"], size=matplotlib.rcParams["font.size"], color=matplotlib.rcParams["text.color"])
        return PlotStyle(color_palette=color_palette, background_color=background_color, line_style=line_style, line_width=line_width, marker_style=marker_style, marker_size=marker_size, grid=grid, font=font)
    ...
    def style(self, style: PlotStyle):
        ...
        matplotlib.rcParams.update({'axes.prop_cycle': cycler('color', style.color_palette)})
        matplotlib.rcParams.update({'axes.edgecolor': 'none'})
        matplotlib.rcParams.update({'axes.axisbelow': True})
        matplotlib.rcParams.update({'axes.grid': True})
        matplotlib.rcParams.update({'axes.facecolor': style.background_color}) # legend background color
        matplotlib.rcParams.update({'figure.facecolor': style.background_color}) # figure background color
        matplotlib.rcParams.update({'savefig.facecolor': style.background_color}) # figure background color
    ...
    def legend(self) -> Legend:
        return Legend(self.axes.get_legend_handles_labels()[1], self.axes.get_legend_loc())
