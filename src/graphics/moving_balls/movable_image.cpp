...
MovableImage::MovableImage(QRectF rectBorder, QObject *parent) : QObject(parent), QGraphicsItem() {
    ...
    auto startX = (std::rand() % (int)(rectBorder.width() - rectSize)) + rectBorder.x();
    auto startY = (std::rand() % (int)(rectBorder.height() - rectSize)) + rectBorder.y();
...
void MovableImage::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
QPointF MovableImage::getSpeed()
{
    return speed;
}


void MovableImage::setSpeed(QPointF newSpeed)
{
    speed = newSpeed;
}

void MovableImage::reverseSpeed()
{
    speed = QPointF(-speed.x(), -speed.y());
}
