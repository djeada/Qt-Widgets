...
BorderedScene::BorderedScene(QRectF borderRect, unsigned int numBalls, QObject *parent) : QGraphicsScene(parent)
{
...
void BorderedScene::advance(){
   ...
   // check for collisions
     for (auto it = objectsToCheck.constBegin(); it != objectsToCheck.constEnd(); ++it) {
     ...
     for (auto otherItem :collidingItems(comparedObject)) {
       ...
       while (comparedObject->collidesWithItem(otherObject)) {
         ...
         comparedObject->setPos(comparedObject->pos() + comparedObject->getSpeed());
       }
