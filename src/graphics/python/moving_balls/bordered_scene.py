...
class BorderedScene(QGraphicsScene):
    ...
    def drawSceneBorder(self):
        ...
        bottom_line = QLineF(
            self.sceneRect().bottomLeft(), self.sceneRect().bottomRight()
        )
    ...
    def advance(self):
        ...
        for compared_object, value in objects_to_check.items():
            ...
            for other_item in self.collidingItems(compared_object):
                ...
                if (
                    other_item is None
                    or not isinstance(other_item, MovableImage)
                    or not objects_to_check[other_item]
                ):
                    continue
                ...
                compared_object.speed = QPointF(
                    abs(compared_object.speed.x()) * other_item_speed_dir.x(),
                    abs(compared_object.speed.y()) * other_item_speed_dir.y(),
                )
                other_item.speed = QPointF(
                    abs(other_item.speed.x()) * compared_object_speed_dir.x(),
                    abs(other_item.speed.y()) * compared_object_speed_dir.y(),
                )

                # move the objects so they don't overlap
                compared_object.moveBy(
                    compared_object.speed.x(), compared_object.speed.y()
                )
