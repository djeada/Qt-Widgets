...
ClickableTreeView::ClickableTreeView(QWidget *parent)
    : QTreeView(parent) {

   setContextMenuPolicy(Qt::CustomContextMenu);
    connect(this, &ClickableTreeView::customContextMenuRequested, this,[&](const QPoint &pos) {
...
void ClickableTreeView::addChild(QModelIndex index) {
  ...
  model->itemFromIndex(index)->appendChild(new TreeItem(QStringList() << "New Item", model->itemFromIndex(index)));
  expandAll();
}
...
void ClickableTreeView::removeItem(QModelIndex index) {
  auto model = this->model();
  model->itemFromIndex(index)->parent()->removeChild(model->itemFromIndex(index));
  expandAll();
}


void ClickableTreeView::editItem(QModelIndex index) {
  edit(index);
}
