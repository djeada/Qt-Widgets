...
QString FileDirModel::getUniqueName(const QString &dirPath, const QString &name)
{
...
QModelIndex FileDirModel::mkdir(const QModelIndex &parent, const QString &name)
{
    // check if index represents a directory if not climb up till we find one
...
bool FileDirModel::copyFile(const QString &sourcePath, const QString &destPath)
{
...
bool FileDirModel::copyDirectory(const QString &sourcePath, const QString &destPath)
{
...
bool FileDirModel::paste(const QModelIndex &index, const QString &path){
    ...
    if (QFileInfo(path).isFile()) {
        auto uniqueName = getUniqueName(fileInfo(parentIndex).absoluteFilePath(), QFileInfo(path).fileName());
