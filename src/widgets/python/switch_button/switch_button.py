...
class SwitchButton(QCheckBox):
    ...
    def paintEvent(self, event):
        ...
        background_color = self.palette().dark().color() if self.isChecked() else self.palette().highlight().color()

        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.setPen(QPen(background_color))
        painter.setBrush(background_color)
        painter.drawRoundedRect(x_rect, y_rect, rect_width, rect_height, 0.5 * rect_height, 0.5 * rect_height)
